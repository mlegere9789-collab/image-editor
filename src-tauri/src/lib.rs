...
fn apply_image(
    ...
    target_channel: Option<document::ApplyTarget>,
    opacity: u8,
    invert: bool,
    ...
    edit_checkpointed(&state, |document| {
        document.apply_image_with(
            ...
            blend,
            mask,
            target_channel.unwrap_or(document::ApplyTarget::Rgb),
            opacity,
            invert,
...
/// Filter Gallery > Blur Gallery > Field Blur on layer `id`: `pins`, each
/// an `(x, y, radius)` triple, at least one.
#[tauri::command]
fn field_blur(
    state: State<'_, AppState>,
    id: LayerId,
    ...
    edit_checkpointed(&state, |document| document.field_blur_with(id, &pins))
}
